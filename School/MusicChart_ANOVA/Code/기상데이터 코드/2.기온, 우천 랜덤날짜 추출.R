...
#경로 지정
setwd('C:/Users/senio/OneDrive/바탕 화면/실험계획법 기말/기상 데이터')
...
#필요한 열만 남기고 삭제
data <- combined_data %>%
  select(date = "일시", avg_temp = "평균기온..C.", daily_precip = "일강수량.mm.", 
         avg_cloud = "평균.전운량.1.10.", snow = "일.최심적설.cm.")
...
#비, -10°C ~ 0°C인 날짜 각 년도에서 하나씩 추출
filtered_data <- data %>%
  filter(avg_temp >= -10, avg_temp < 0, daily_precip >= 5, is.na(snow))
...
#비, 0°C ~ 10°C인 날짜 각 년도에서 하나씩 추출
filtered_data <- data %>%
  filter(avg_temp >= 0, avg_temp < 10, daily_precip >= 5, is.na(snow))
...
#비, 10°C ~ 20°C인 날짜 각 년도에서 하나씩 추출
filtered_data <- data %>%
  filter(avg_temp >= 10, avg_temp < 20, daily_precip >= 5, is.na(snow))
...
#비, 20°C ~ 30°C인 날짜 각 년도에서 하나씩 추출
filtered_data <- data %>%
  filter(avg_temp >= 20, avg_temp < 30, daily_precip >= 5, is.na(snow))
