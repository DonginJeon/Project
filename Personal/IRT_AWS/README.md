...
- Public 및 Private으로 서브넷을 나누어 10.0.100.0/24와 10.0.101.0/24로 설정했다.
  > 빅데이터 과정이 사용하는 AWS 계정에서 사용가능한 포트로서 100, 101을 선정.
...
### 개선 이유
1. 문항모수가 있는 문제 업데이트
  - 기존 문항모수가 있는 문제는 일정 주기가 되었을때 EM알고리즘에서 m-step과정을 참고하여 적재된 학생 능력치를 통해 문항모수를 업데이트 진행. 
...
1. **문제 출제 및 S3 업로드**  
   ...
   - 문제 설명, 정답, 학습 목표 포함 (난이도, 변별도 정보 없음)
...
3. **EC2에서 문제 파일 다운로드 및 전처리**  
   ...
   - 사전 분포 설정 준비 
    \[
    a(\text{난이도}) \sim \mathcal{N}(0.5, 0.1)
    \]
