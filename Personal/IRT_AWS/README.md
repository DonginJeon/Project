...
1. 파이썬 패키지를 활용해 AWS 환경에 최적화된 시스템을 구축
...
- **평가**: 데이터 및 모델에 대한 이해 부족으로 Lambda에서 θ 값을 처리하는 과정이 구체적이지 못함.

  ![alt text](image-1.png)

#### 개선 버전 1

- **평가**: Lambda를 활용하여 RDS에 θ 값을 저장하고 업데이트하는 구조로 변경. 초기 버전에 비해서는 구체적으로 변화하였지만 여전히 모델을 적용시키는 것에는 이해가 부족했다고 평가. 특히 람다를 어떻게 사용할지 Sagemaker에 대한 구체성이 부족하고 Sagemaker의 비용문제에 대한 고려가 부족했음.
...
- **시도**: SageMaker를 제거하고, Lambda 기능을 세 개의 함수로 분산하여 유지 보수성 및 비용 절감 효과를 높였다. API-gateway를 활용하여 응답데이터를 통해 θ 값을 업데이트하고 문제를 반환받은 뒤 반복하여 업데이트되는 θ 값을 RDS에 적재하는 작업까지 시도
  ...
  > 아래에서 실제 어떻게 테스트했는지 과정을 넣음.(AWS 환경 구축~)
...
- EM 알고리즘을 효율적으로 적용하여 학생 능력치와 문항 모수를 관리할 수 있도록 하는 시스템 구축을 시도
