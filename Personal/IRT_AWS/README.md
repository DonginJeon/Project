...
- 대시보드 툴을 통해 θ 값 시각화를 최적화하였다.
- **EM 알고리즘**을 통한 θ 값과 문항 모수 업데이트 방안을 검토 중이다.
- Lambda 함수를 개선하여 각 기능이 더욱 안정적으로 작동하도록 최종 점검을 완료하였다.
...
1. **Lambda를 통한 분산 처리**: 분산 구조로 Lambda를 설정하니 유지보수와 비용 절감 효과가 컸다.
    > 의구심 : 천재교육에서 Lambda를 활용할 때 기능을 최대한 분산하여 사용하는가? 아님 통합 하여 사용하는가?
2. **AWS 서비스 통합**: RDS와 API Gateway, Lambda의 조합을 통해 실시간 데이터 업데이트 및 학생별 문제 추천을 효율적으로 구현할 수 있었다.
